...
export class EquiposController{
    ...
    create = ( req:Request, res: Response ) => {
        ...
        this.equiposService.create( createEquipoDto! )
        .then( equipos => res.json(equipos) )
        .catch(error => res.status(500).json({error}));
    }

    update = ( req:Request, res: Response ) => {
        const [error, updateEquipoDto] = UpdateEquipoDto.update( {...req.body, id_equipo: req.params.id_equipo} );
        if( error ) return res.status(400).json({error})
        
            this.equiposService.update( updateEquipoDto! )
            .then( equipos => res.json(equipos) )
            .catch(error => res.status(500).json({error}));
